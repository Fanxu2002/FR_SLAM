...
#include "fr_slam/mapping/incremental_global_map.hpp"
#include "fr_slam/mapping/keyframe_detector.hpp"
#include "fr_slam/mapping/keyframe_manager.hpp"
#include "fr_slam/lidar/lidar_registration.hpp"
#include "fr_slam/lidar/lidar_registration_config.hpp"
#include "fr_slam/loop/loop_detector.hpp"
#include "fr_slam/loop/loop_verifier.hpp"
#include "fr_slam/loop/loop_consistency.hpp"
#include "fr_slam/common/point_types.hpp"
#include "fr_slam/backend/pose_graph.hpp"
...
//         Finished Submap -> BTC historical database
//
...
struct LoopRuntimeConfig
{
    ...
    // BTC official profile selection and FR-SLAM integration parameters.
    // "indoor"  -> config_indoor.yaml
    // "outdoor" -> config_outdoor.yaml
    std::string btc_config_profile = "outdoor";
    int btc_skip_near_num = 30;
    int btc_proj_plane_num = 3;
...
private:
    // Debug-only planar-motion assumption for indoor loop-closure tests.
    // When enabled, accepted frontend poses are projected to x/y/yaw only:
    // z = 0, roll = 0, pitch = 0 in the initial LiDAR/world frame.
    bool planar_motion_mode_ = false;
    ...
    LoopConsistencyChecker loop_consistency_checker_;

    // Legacy online-loop tuning retained for compatibility with existing
    // backend safety limits.  Global retrieval itself is BTC-only.
    ...
    // Tentative track grace window retained for backend state compatibility.
    ...
    // candidate-anchor jitter in the opposite direction.
    ...
    // batch AND that anchor has raw BTC retrieval provenance, the immediately
    // following geometrically consistent observation may use the adaptive
    // continuation cap.  This fixes the V19.5 dead-zone:
    //
    //     KF146->94  = 19.059 m  (strict independent-retrieval seed, PASS)
    ...
    // neighborhood instead of waiting for BTC retrieval to independently return
    ...
    //
    // The first LARGE_DRIFT anchor MUST be independently seeded by independent BTC retrieval.  Follow-up anchors may come from TRACK/neighborhood injection
    ...
    std::size_t online_loop_large_drift_retrieval_support_radius_ = 2;
    std::size_t online_loop_large_drift_min_retrieval_members_ = 1;
    ...
    // An injected large-drift follow-up must either be supported by independent BTC retrieval OR lie inside the corrected-space revisit corridor.  Geometry
    ...
    // Normal BTC retrieval discovery still verifies only one candidate per
    // historical Submap.
    ...
    // ------------------------------------------------------------------------
    // Complete runtime loop/BTC configuration used by loop_closure.cpp.
    LoopRuntimeConfig loop_runtime_config_;
    ...
    struct PendingLoopConstraint
    {
        ...
        // Independent-retrieval provenance retained for backend diagnostics.
        // In the BTC-only pipeline this means the proposal came directly from
        // BTC rather than from a propagated local-track hypothesis.
        bool independent_retrieval_supported = false;
        std::size_t independent_retrieval_support_kf =
            std::numeric_limits<std::size_t>::max();
        double independent_retrieval_support_score = 0.0;
    };
    ...
    //     historical KFs so BTC retrieval does not have to rediscover each one,
