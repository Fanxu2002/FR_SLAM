...
#include "fr_slam/imu/imu_initializer.hpp"
...
private:
    ...
    rclcpp::Publisher<
        visualization_msgs::msg::MarkerArray>::SharedPtr
        multi_plane_marker_pub_;

    std::size_t last_published_multi_plane_revision_ = 0;
...
private:
    ...
    // ============================================================
    void PublishMultiPlaneMarkers(
        const builtin_interfaces::msg::Time &stamp,
        const std::string &lidar_frame)
    {
        if (!multi_plane_marker_pub_)
        {
            return;
        }

        fr_slam::MultiPlaneSnapshot snapshot;

        if (!fr_slam::GetLatestMultiPlaneSnapshot(snapshot) ||
            snapshot.revision ==
                last_published_multi_plane_revision_)
        {
            return;
        }
        ...
        clear_marker.header.frame_id = lidar_frame;
        ...
        for (std::size_t plane_index = 0;
             plane_index < snapshot.result.planes.size();
             ++plane_index)
        {
            const fr_slam::PlaneObservation &plane =
                snapshot.result.planes[plane_index];

            if (!plane.center_L.allFinite() ||
                !plane.normal_L.allFinite() ||
                plane.normal_L.norm() < 1.0e-9)
            {
                continue;
            }

            // Clean V1 visualization:
            //   1) show only the currently selected Ground;
            //   2) show only Wall candidates that also satisfy the
            //      association quality threshold.
            //
            // NOTE:
            // wall_constraint_candidate is still a candidate-level flag.
            // ACTIVE_STATIC lives in the Wall Association V3.1 layer and is
            // intentionally NOT faked here. When Plane Pose Constraint is
            // connected, that state will be bridged explicitly.
            const bool show_ground =
                plane.active_ground;

            const bool show_wall =
                plane.wall_constraint_candidate &&
                std::isfinite(
                    plane.constraint_quality_score) &&
                plane.constraint_quality_score >= 0.80;
            ...
            Eigen::Vector3d normal_L =
                plane.normal_L.normalized();

            // Make the Ground arrow visually point upward.
            if (show_ground &&
                normal_L.dot(
            ...
            const float red =
                show_ground ? 0.10F : 0.10F;

            const float green =
                show_ground ? 0.90F : 0.55F;

            const float blue =
                show_ground ? 0.20F : 1.00F;

            const float surface_alpha =
                show_ground ? 0.30F : 0.22F;

            // -------------------------------------------------------------
            // Plane surface
            // -------------------------------------------------------------
            visualization_msgs::msg::Marker surface;
            surface.header.frame_id = lidar_frame;
            surface.header.stamp = stamp;
            surface.ns =
                show_ground ?
                    "constraint_ground_surface" :
                    "constraint_wall_surface";
            ...
            surface.pose.position.x =
                plane.center_L.x();
            surface.pose.position.y =
                plane.center_L.y();
            surface.pose.position.z =
                plane.center_L.z();

            surface.pose.orientation.x =
                orientation.x();
            surface.pose.orientation.y =
                orientation.y();
            surface.pose.orientation.z =
                orientation.z();
            surface.pose.orientation.w =
                orientation.w();
            ...
            surface.scale.y =
                2.0 * visual_radius;
            surface.scale.z =
                show_ground ? 0.025 : 0.035;

            surface.color.r = red;
            surface.color.g = green;
            surface.color.b = blue;
            surface.color.a = surface_alpha;
            ...
            arrow.header.frame_id = lidar_frame;
            arrow.header.stamp = stamp;
            arrow.ns =
                show_ground ?
                    "constraint_ground_normal" :
                    "constraint_wall_normal";
            arrow.id = marker_id++;
            arrow.type =
                visualization_msgs::msg::Marker::ARROW;
            arrow.action =
            ...
            p0.x = plane.center_L.x();
            p0.y = plane.center_L.y();
            p0.z = plane.center_L.z();

            const double arrow_length =
                show_ground ? 0.65 : 0.55;

            geometry_msgs::msg::Point p1;
            p1.x =
                plane.center_L.x() +
                arrow_length * normal_L.x();
            p1.y =
                plane.center_L.y() +
                arrow_length * normal_L.y();
            p1.z =
                plane.center_L.z() +
                arrow_length * normal_L.z();

            arrow.points.push_back(p0);
            arrow.points.push_back(p1);

            // Thicker than the old debug arrows so the actual constraint
            // directions remain visible over the point cloud.
            arrow.scale.x = 0.075;
            arrow.scale.y = 0.140;
            arrow.scale.z = 0.180;
            ...
            label.header.frame_id = lidar_frame;
            label.header.stamp = stamp;
            label.ns =
                show_ground ?
                    "constraint_ground_label" :
                    "constraint_wall_label";
            label.id = marker_id++;
            label.type =
            ...
            label.pose.position.z =
                plane.center_L.z() +
                (show_ground ? 0.28 : 0.32);

            label.pose.orientation.w = 1.0;

            label.scale.z =
                show_ground ? 0.22 : 0.20;
        ...
        multi_plane_marker_pub_->publish(
            marker_array);
    }
...
public:
              ...
              "scan2local_map")
    {
        ...
        std::transform(
            ...
            ground_constraint_config.mode.end(),
            ground_constraint_config.mode.begin(),
            [](unsigned char character)
            {
                return static_cast<char>(
        ...
        ground_constraint_config.analysis_voxel_leaf_m =
            std::max(
                0.03,
                this->declare_parameter<double>(
                    "ground_analysis_voxel_leaf_size",
                    0.15));

        ground_constraint_config.anchor_bootstrap_frames =
            static_cast<std::size_t>(
                std::max(
                    1L,
                    this->declare_parameter<int>(
                        "ground_anchor_bootstrap_frames",
                        8)));

        ground_constraint_config.anchor_maximum_normal_spread_deg =
            std::max(
                0.1,
                this->declare_parameter<double>(
                    "ground_anchor_max_normal_spread_deg",
                    2.0));

        ground_constraint_config.anchor_maximum_plane_d_spread_m =
            std::max(
                0.005,
                this->declare_parameter<double>(
                    "ground_anchor_max_plane_d_spread_m",
                    0.08));

        ground_constraint_config.plane_information_scale =
            std::max(
                0.0,
                this->declare_parameter<double>(
        ...
        ground_constraint_config
            ...
            std::max(
                0.0,
                this->declare_parameter<double>(
                    "ground_max_general_rmse_absolute_increase_m",
                    0.003));

        ground_constraint_config.minimum_general_correspondence_ratio =
            std::clamp(
                this->declare_parameter<double>(
                    "ground_min_general_correspondence_ratio",
                    0.85),
                0.0,
                1.0);
        ...
        ground_constraint_config.diagnostics_enabled =
            this->declare_parameter<bool>(
                "ground_diagnostics_enable",
                true);

        fr_slam::GroundSegmentationConfig &ground_segmentation_config =
            ground_constraint_config.segmentation;

        ground_segmentation_config.support_constraint_minimum_points =
            static_cast<std::size_t>(
                std::max(
                    1L,
                    this->declare_parameter<int>(
                        "ground_support_constraint_minimum_points",
                        120)));

        ground_segmentation_config.support_constraint_minimum_cells =
            static_cast<std::size_t>(
                std::max(
                    1L,
                    this->declare_parameter<int>(
                        "ground_support_constraint_minimum_cells",
                        35)));
        ...
        ground_segmentation_config
            ...
            static_cast<std::size_t>(
                std::max(
                    1L,
                    this->declare_parameter<int>(
                        "ground_support_constraint_minimum_center_cells",
                        3)));

        ground_segmentation_config.support_constraint_minimum_inlier_ratio =
            std::clamp(
                this->declare_parameter<double>(
                    "ground_support_constraint_minimum_inlier_ratio",
                    0.90),
                0.0,
                1.0);

        ground_segmentation_config.support_constraint_maximum_rmse_m =
            std::max(
                0.001,
                this->declare_parameter<double>(
                    "ground_support_constraint_maximum_rmse_m",
                    0.030));

        ground_segmentation_config.support_constraint_minimum_selected_score =
            this->declare_parameter<double>(
                "ground_support_constraint_minimum_score",
                8.5);

        ground_segmentation_config.support_constraint_enter_confidence =
            std::clamp(
                this->declare_parameter<double>(
                    "ground_support_constraint_enter_confidence",
                    0.72),
                0.0,
                1.0);

        ground_segmentation_config.support_constraint_keep_confidence =
            std::clamp(
                this->declare_parameter<double>(
                    "ground_support_constraint_keep_confidence",
                    0.62),
                0.0,
                ground_segmentation_config
                    .support_constraint_enter_confidence);

        ground_segmentation_config.support_clearance_bootstrap_samples =
            static_cast<std::size_t>(
                std::max(
                    1L,
                    this->declare_parameter<int>(
        ...
        //
        // lo.launch.py sets FR_SLAM_OUTPUT_DIR and also passes explicit
