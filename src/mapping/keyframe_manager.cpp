...
bool KeyframeManager::AddKeyframe(
    ...
    const pcl::PointCloud<LIDAR_POINT>::ConstPtr &btc_dense_cloud)
{
    // ------------------------------------------------------------------------
    ...
    // ------------------------------------------------------------------------
    // 4.1 Keep a second, denser snapshot for BTC retrieval only.
    //
    // Prefer the Basic/ROI/CropBox cloud captured before the normal
    // registration Voxel/SOR/ROR chain. If the bridge is unavailable, fall
    // back to the ordinary Keyframe cloud so the rest of FR-SLAM still runs.
    // ------------------------------------------------------------------------
    const pcl::PointCloud<LIDAR_POINT>::ConstPtr btc_source_cloud =
        (btc_dense_cloud && !btc_dense_cloud->empty())
            ? btc_dense_cloud
            : cloud_lidar;
