...
namespace
{
        ...
        const LoopVerifierConfig &config)
    {
        ...
        // a trusted reverse temporal sequence has been confirmed by the caller.
    ...
    LoopVerificationResult &result) const
{
    ...
    const void *source_identity =
        static_cast<const void *>(source_current.get());
        ...
        !cache_->source.filtered)
    {
        ...
        const pcl::PointCloud<pcl::PointXYZ>::Ptr source_filtered =
            VoxelFilter(source_xyz, config_.voxel_leaf_size);
    ...
    prepared_source.cloud = cache_->source.filtered;

    const void *target_identity =
        static_cast<const void *>(target_historical.get());
    ...
    if (!target_cache_entry_valid)
    {
        ...
        const pcl::PointCloud<pcl::PointXYZ>::Ptr target_filtered =
            VoxelFilter(target_xyz, config_.voxel_leaf_size);
        ...
        while (cache_->targets.size() >= config_.max_cached_targets &&
               !cache_->target_order.empty())
        {
            const void *oldest_identity = cache_->target_order.front();
            cache_->target_order.pop_front();
            cache_->targets.erase(oldest_identity);
        }

        Cache::TargetEntry entry;
        entry.original_size = target_historical->size();
        entry.prepared.cloud = target_filtered;
        entry.prepared.kdtree = target_kdtree;

        cache_->targets.emplace(target_identity, std::move(entry));
        cache_->target_order.push_back(target_identity);
        target_iterator = cache_->targets.find(target_identity);
    }
    ...
    LoopVerifierHypothesis hypothesis)
{
    switch (hypothesis)
    {
    case LoopVerifierHypothesis::ExternalSeed:
